...
export default function DeliveryProcedureCreatePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    // // Additional delivery dates from lot
  ...
  const handleLotChange = (lotId: string) => {
    ...
    if (selectedLot) {
      setFormData({
        ...
        delivery_date: selectedLot.actual_delivery_date || '',
        delivery_date_smart_meters: selectedLot.meter_delivery_date || '',
      })
    } else {
      setFormData({
        ...formData,
  ...
  // 1. Create a Set to track the lot numbers we've already seen
const seenLotNumbers = new Set();

// 2. Filter the array to only include the first instance of each lot number
const uniqueLots = availableLots.filter(lot => {
  // Use 'N/A' as the fallback, matching your UI logic
  const lotNumber = lot.item_lot_no || 'N/A'; 
  
  if (seenLotNumbers.has(lotNumber)) {
    return false; // We've seen this lot number before, so filter it out
  }
  
  seenLotNumbers.add(lotNumber); // First time seeing it, add to Set
  return true; // Keep this lot in the array
});
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                readOnly={!formData.delivery_date} // Make read-only if ATA is not set
                // onChange={(e) => setFormData({ ...formData, delivery_date: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-100 text-gray-700" // Gray out if read-only
            ...
            <div>
              ...
              <input
                ...
                readOnly={!formData.delivery_date_smart_meters} // Make read-only if delivery_date is not set

...
export default function PtcEditPage() {
  ...
  const [ptcSentDate, setPtcSentDate] = useState('')
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // 1) Load PTC
        const ptcUrl = `${API}/pre_tender_clarification/${id}`
        ...
        setPtcSentDate(ptcData.ptc_sent_date ?? '')
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        ptc_sent_date: ptcSentDate || null,
        // ptc_reply_submission_date: ptcReplySubmissionDate || null
      }

      const url = `${API}/pre_tender_clarification/${ptc.ptc_id}`
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <h1 className="text-2xl font-bold">Edit Pre Tender Clarification #{ptc.ptc_id}</h1>
      ...
      <form onSubmit={handleSave} className="space-y-4 rounded-md border p-4">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button
            type="button"
            onClick={() => router.push('/dashboard/pre_tender_clarification')}

...
export default function MPGEditPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/material_performance_guarantee/${id}`, { headers: { Authorization: `Bearer ${token}` } }),
      fetch(`${API}/order_detail`, { headers: { Authorization: `Bearer ${token}` } })
    ])
      .then(async ([mpgRes, ordersRes]) => {
        if (!mpgRes.ok) throw new Error('Failed to fetch MPG data')
        const data = await mpgRes.json()
        ...
        setFormData({
          ...
          mpg_no: data.mpg_no || '',
          participated: data.participated === 1,
          mpg_bank_or_deposit: data.mpg_bank_or_deposit || 0,
          mpg_issuing_bank: data.mpg_issuing_bank || '',
          mpg_deposit_receipt_no: data.mpg_deposit_receipt_no || '',
          mpg_value: data.mpg_value ? formatNumber(String(data.mpg_value)) : '',
          mpg_expiry_date: formatDateString(data.mpg_expiry_date),
          mpg_submitted_date: formatDateString(data.mpg_submitted_date),
          mpg_release_date_dewa: formatDateString(data.mpg_release_date_dewa),
          mpg_release_date_bank: formatDateString(data.mpg_release_date_bank),
          mpg_extension_dates: data.mpg_extension_dates || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API}/material_performance_guarantee/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        setError(err?.detail || 'Failed to update MPG')
      }
    } catch (e) {
      console.error(e)
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold">Edit Material Performance Guarantee</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          >
            ...
            {saving ? 'Saving...' : 'Update Guarantee'}
          </button>
        </div>
      </form>
    </div>
